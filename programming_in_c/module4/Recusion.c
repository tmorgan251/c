...
You should write a recursive function called sumseries() that takes as its input a single integer and
returns the sum of all the numbers from 1..the provided number. This is very similar to the function
described in lecture except for some error checking. First, if the provided number is less than one,
return zero as the sum. Also, in order to avoid stack overflow, if the provided number is above 100 return -1.
